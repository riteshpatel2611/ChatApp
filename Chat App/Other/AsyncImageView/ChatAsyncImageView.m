                ...
                if (imageProperties)
                {
                    ...
                    if (imageRef)
                    {
                        ...
                        if (image && self.URL)
                        {
                            ...
                            [cache storeImage:image forUrl:self.URL.absoluteString];
...
@implementation ChatAsyncImageView
...
- (UIImage *)getImageForUrl:(NSString *)sUrl {
    
    if (sUrl.length == 0) {
        return nil;
    }
    ImageCache *cache = [ImageCache sharedImageCache];
    if([cache isImageExistsInCache:sUrl]) {
        UIImage *img = [cache imageForUrl:sUrl];
        return img;
    }
    return nil;
}
...
- (void)setImageURL:(NSURL *)imageURL
{
    UIImage *image = [self getImageForUrl:imageURL.absoluteString];
    //NSLog(@"image = %@", image);
    if (image)
    {
        self.image = image;
