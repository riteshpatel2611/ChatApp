                                                  ...
                                                  managedObjectContext:(NSManagedObjectContext *)moc
{
    ...
    //predicate = [NSPredicate predicateWithFormat:@"bareJidStr == 'jeet@52.65.0.92' AND streamBareJidStr == 'abc@52.65.0.92'"];
...
- (void)archiveMessage:(XMPPMessage *)message outgoing:(BOOL)isOutgoing xmppStream:(XMPPStream *)xmppStream
{
    ...
    //NSLog(@"archiveMessage = %@ isOutgoing = %d", message, isOutgoing);
    
    if ([[message type] isEqualToString:@"groupchat"] && isOutgoing == 0 && [[[message to] bare] isEqualToString:xmppStream.myJID.bare] && [[[[message to] bare] removeHostName] isEqualToString:[[message fromStr] lastPathComponent]]) {
        //NSLog(@"chal bhag!!!");
        return;
    }
...
- (void)updateLastSeen:(NSDate *)lastSeenTimeStamp bareId:(XMPPJID *)bareJId streamBareId:(XMPPJID *)streamBareJId {
    
    [self scheduleBlock:^{
        
        NSManagedObjectContext *moc = [self managedObjectContext];
        BOOL didCreateNewContact = NO;
        XMPPMessageArchiving_Contact_CoreDataObject *contact = [self contactWithJid:bareJId streamJid:streamBareJId managedObjectContext:moc];
        if (contact == nil) {
        ...
        contact.streamBareJidStr = streamBareJId.bare;
        contact.bareJidStr = bareJId.user;
        contact.bareJid = bareJId;
        contact.lastSeenTimeStamp = lastSeenTimeStamp;
