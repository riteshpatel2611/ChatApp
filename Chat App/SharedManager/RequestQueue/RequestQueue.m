...
- (void)addRequest:(NSURLRequest *)request ReqiestId:(NSString *)reqId completionHandler:(RQCompletionHandler)completionHandler
{
    RQOperation *operation = [RQOperation operationWithRequest:request];
    operation.completionHandler = completionHandler;
    operation.requestId = reqId;
...
- (BOOL)isRequestInProgressForId:(NSString *)requestId
{
    for (RQOperation *op in [self.operations reverseObjectEnumerator])
    {
	  if ([op.requestId isEqualToString:requestId])
	  {
		return YES;
	  }
    }
